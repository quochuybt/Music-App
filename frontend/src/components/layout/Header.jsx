...
export default function Header({ onMenu }) {
  ...
  return (
    <header className="app-header sticky top-0 z-30 border-b border-white/10 bg-[#080b10]/82 backdrop-blur-xl">
      <div className="flex h-[72px] min-w-0 items-center gap-3 px-4 py-3 lg:px-8">
        ...
        {isAuthenticated ? (
          <div className="ml-auto flex shrink-0 items-center gap-2">
            {user?.role === "ADMIN" && <Link to="/admin/dashboard" className="hidden sm:block"><Button variant="ghost"><Shield size={16} /> Admin</Button></Link>}
